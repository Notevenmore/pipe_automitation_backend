...
class DE_Best_1: 
    def __init__(self, ps, length_b1_b2, length_b1_b3, ps1, ps2):
        ...
        best_b1_b2 = self.getBestB1B2()
        best_b1_b3 = self.getBestB1B3()
        self.best_all = np.concatenate((best_b1_b2[0],best_b1_b3[0][3:]))
        self.K = self.getCompressor()
    ...
    def getCompressor(self): 
        ...
        for i in range(len(self.best_all)-1):   
            if i == 0: K.append(self.best_all[i][0] / self.ps)  
                ...
                K.append(self.best_all[i][0] / self.best_all[i-1][1])

        return K
    
    def getBestPipeB1B2(self):
        while True:
    ...
    def getBestB1B2(self):
        best_pipe_b1_b2 = self.getBestPipeB1B2()
        while(True):
    ...
    def getBestB1B3(self):
        best_b1 = self.getBestB1()
        while(True):
            ...
            if(round(best_b1_b3[0][self.N1 + self.N3 - 1][1]) == self.ps2):
                status = True
            if status and (best_b1_b3[0][:3] == self.best_b1).all() and self.check_inequality_2(best_b1_b3[0]):
                break
        ...
        for i in range(self.iter):
            ...
            for j in range(pop_size):
                ...
                while(True):
                    ...
                    mutated_b1 = self.check_bounds(mutated[:3], self.bounds[0])
                    mutated_b2 = self.check_bounds(mutated[3:], self.bounds[1])
    ...
    def get_pipe_length2(self, pipe_b3, best_b1, N1, N3):
        best_pipe = []
        for i in range(self.pop_size): # 1, 2, ..., 50
            ...
            for j in range(self.N1 + self.N3): # 1, 2, 3, 4, 5, 6, 7
                ...
                if j >= self.N1-1: # 3, 4, 5, 6 => 0, 1, 2, 3
                    sum_pipe += self.pipe_b3[j- self.N3][i][2]
    ...
    def create_pipe_b3(self):
        while True:
            ...
            best_pipe_b3 = self.get_pipe_length2(pipe_b3, self.best_b1, self.pop_size, self.N1, self.N3)
